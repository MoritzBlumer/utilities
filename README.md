...
- ```vcf_pca.py stackplot``` likewise reads these output files and creates an interactive HTML plot (and/or regular PDF, see ```-f```). ```-p```. It visualizes all 10 PCs stacked on top of each other along with the variance explained by each. Individual samples are connected with lines so that they can be traced along the PCs. Optionally, PC values are scaled according to the amount of variance they explain (```--s```). Use ```-m``` to supply a metadata file which will be used to annotate the HTML plot (the first column must contain the same sample IDs as used in the VCF file). See ```vcf_pca.py plot -h``` for more option, including reflecting data along PC axes, setting plot colors for groups of samples or using a continuous color scheme based on a numeric metadata column.
